...
export default async function BlogListPage({ params }: PageProps) {
  ...
  // API not ready / network error — don't cache this render so next request retries
  ...
  return (
    <>
      ...
      <div className="max-w-[1400px] mx-auto px-6 nav:px-10 py-12">
        {posts.length === 0 ? (
          <p className="text-secondary-text text-center py-20">{locale === 'tr' ? 'Henüz yazı yok.' : 'No posts yet.'}</p>
        ) : (
          <>
