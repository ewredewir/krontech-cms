...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://krontech.com/en/about-us`,
      languages: {
...
export default async function AboutEnPage({ params }: PageProps) {
  const locale = params.locale as Locale;
            ...
            {(['mission', 'vision', 'values'] as const).map((key) => (
              <section key={key} aria-label={t(key)} className="bg-white shadow-card p-6">
                ...
                <p className="text-secondary-text text-body">
                  {key === 'mission' && 'Helping organizations complete their digital transformation securely by protecting them from cyber threats.'}
                  {key === 'vision' && 'To become a globally recognized domestic cybersecurity company.'}
                  {key === 'values' && 'Integrity, innovation, customer focus, and continuous improvement.'}
                </p>
              </section>
            ))}
          </div>
